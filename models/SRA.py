...
class SRA1(nn.Module):

    def __init__(self, inplanes, num):

        super(SRA1, self).__init__()
    ...
    def forward(self, feat_map, re_featmap, Embeding_feature, feat_vect, aggregative_feature=None):
        ...
        for i in range(0, t, 3):
            ...
            para_0 = para_0 * para_spa[:, i, :, :, :]
            para_1 = para_1 * para_spa[:, i + 1, :, :, :]
            print(para_0.shape,para_1.shape,aggregative_feature[:, int(i/2), :, :, :].shape,self.relu(para_0 * feat_map[:, i, :, :, :] + para_1 * feat_map[:, i + 1, :, :, :]).shape)
            aggregative_feature_list.append(aggregative_feature[:, int(i/2), :, :, :] + self.relu(para_0 * feat_map[:, i, :, :, :] + para_1 * feat_map[:, i + 1, :, :, :]))
...
class SRA2(nn.Module):
    ...
    def forward(self, feat_map, re_featmap, Embeding_feature, feat_vect, aggregative_feature=None):
        ...
        for i in range(0, t, 3):
            ...
            para_0 = para_0 * para_spa[:, i, :, :, :]
            para_1 = para_1 * para_spa[:, i + 1, :, :, :]
            # print(para_0.shape,para_1.shape,aggregative_feature[:, int(i/3), :, :, :].shape)
            # print((para_0 * feat_map[:, i, :, :, :]).shape,'1' )
            # print((para_1 * feat_map[:, i + 1, :, :, :]).shape,'2')
            aggregative_feature_list.append(aggregative_feature[:, int(i/3), :, :, :] + self.relu(para_0 * feat_map[:, i, :, :, :] + para_1 * feat_map[:, i + 1, :, :, :]))
