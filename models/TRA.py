...
class TRA(nn.Module):
    ...
    def forward(self, featmap, re_featmap, vect_featmap, embed_feat):
        ...
        #print(featmap.size(),'featmap.size()1')#torch.Size([4, 8, 1024, 7, 7]) featmap.size()1
        ...
        for idx in range(0, t, 2):
            ...
            #print(Gs_in0.shape,'Gs_in0.shape')#torch.Size([4, 49, 7, 7]) Gs_in0.shape
            ...
            #print(Gs_out1.shape,'Gs_out1.shape' )#torch.Size([4, 49, 7, 7]) Gs_out1.shape
...
class TRA1(nn.Module):

    def __init__(self, inplanes, num):

        super(TRA1, self).__init__()
    ...
    def forward(self, featmap, re_featmap, vect_featmap, embed_feat):
        ...
        b, t, c, h, w = featmap.size()#torch.Size([8, 3, 1024, 7, 7]) featmap.size()1
        ...
        channel_para = self.theta_channel(vect_featmap.permute(0, 2, 1))
        gap_feat_map0 = []
        # print(channel_para.shape,'channel_para.shape')
        # print(t)
        ...
        return gap_feat_map0
class TRA2(nn.Module):

    def __init__(self, inplanes, num):

        super(TRA2, self).__init__()
    ...
    def forward(self, featmap, re_featmap, vect_featmap, embed_feat):
        ...
        # print(featmap.size(),'featmap.size()1')#torch.Size([8, 12, 1024, 7, 7]) featmap.size()1

        gamma_feat = self.gamma_temporal(re_featmap).view(b, t, -1, h * w)
        # print(gamma_feat.shape,'gamma_feat.shape')#torch.Size([8, 12, 128, 49]) gamma_feat.shape

        beta_feat = self.beta_temporal(re_featmap).view(b, t, -1, h * w)
        # print(beta_feat.shape,'beta_feat.shape')#torch.Size([8, 12, 128, 49]) beta_feat.shape
        channel_para = self.theta_channel(vect_featmap.permute(0, 2, 1))
        gap_feat_map0 = []
        # print(channel_para.shape,'channel_para.shape')#torch.Size([8, 1, 1024, 7, 7]) gap_feat_map0.shape

        # print(t)
        for idx in range(0, t,3):
            # print(idx)
            # print(channel_para[:, :, idx].shape,channel_para[:, :, idx + 1].shape)
            para0 = torch.cat((channel_para[:, :, idx], channel_para[:, :, idx + 1]), 1)
            para_00 = self.channel_para(para0).view(b, -1, 1, 1)
